...
class CourtVisualizer():
    def __init__(self, image):
        ...
        self.set_canvas_background_box_position(image)
    ...
    def set_canvas_background_box_position(self,image):
        image=image.copy()

        self.end_x = image.shape[1] - self.buffer
    ...
    def draw_court(self, image):
        ...
        cv2.line(image, net_start_point, net_end_point, (255, 0, 0), 2)

        return image

    def draw_background_rectangle(self, image):
    ...
    def draw_mini_court(self, image):
        image = self.draw_background_rectangle(image)
        image = self.draw_court(image)
        return image
    ...
    def convert_bounding_boxes_to_mini_court_coordinates(self, player_boxes, ball_boxes, original_court_key_points):
        ...
        for player_id, player_bbox in enumerate(player_boxes, 1):
            ...
            closest_key_point = (original_court_key_points[closest_key_point_index], 
                                original_court_key_points[closest_key_point_index+1])

            # Get Player height in pixels
            bboxes_heights_in_pixels = get_height_of_bbox(player_boxes[player_id-1]['bbox'])
            max_player_height_in_pixels = bboxes_heights_in_pixels
        ...
        if len(ball_boxes)!=0:
            ...
            #closest_player_id_to_ball = min(player_bbox['bbox'], key=lambda x: measure_distance(ball_position, get_center_of_bbox(player_bbox['bbox'])))
    ...
    def draw_points_players_on_mini_court(self, image, players_positons, color=(0,255,0)):
        ...
        x_2=int(x_2)
        y_2=int(y_2)
        cv2.circle(image, (x_1,y_1), 5, color, -1)
